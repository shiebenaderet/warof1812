  ...
  return (
    <div className="min-h-screen bg-war-ink flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl">
        ...
        <div className="flex gap-3 mb-8 w-full max-w-md mx-auto animate-slideup" style={{ animationDelay: '0.1s' }}>
          <input
            ...
            className="flex-1 px-4 py-3 bg-transparent border border-parchment-dark/30 rounded
                       ...
                       focus:outline-none focus:border-war-gold/70 transition-colors"
            maxLength={30}
          />
          <input
            type="text"
        ...
        {/* Saved game */}
        {savedGame && (
          <div className="w-full max-w-md mx-auto mb-8 bg-war-navy/60 backdrop-blur rounded-lg p-5 border border-war-gold/20 shadow-card animate-slideup">
            <p className="text-parchment text-base font-body mb-3">
              Saved campaign: <span className="text-war-gold font-bold">{savedGame.playerName}</span> — Round {savedGame.round}, {savedGame.season}
            </p>
            <div className="flex gap-3 mb-3">
              <button
                onClick={onContinue}
                className="flex-1 py-2.5 bg-war-gold text-war-ink font-display rounded
              ...
              <button
                ...
                className="px-4 py-2.5 border border-parchment-dark/30 text-parchment-dark font-body rounded
                           hover:border-red-400/60 hover:text-red-400 transition-colors cursor-pointer text-sm"
              >
                Delete
              </button>
            </div>
            <div className="flex gap-3 pt-3 border-t border-parchment-dark/20">
              <button onClick={handleExport} className="flex-1 py-2 border border-war-gold/30 text-war-gold/80 font-body rounded hover:bg-war-gold/10 transition-colors cursor-pointer text-sm">
                Export Backup
              </button>
              <button onClick={handleImport} className="flex-1 py-2 border border-parchment-dark/30 text-parchment-dark font-body rounded hover:border-war-gold/40 hover:text-war-gold/80 transition-colors cursor-pointer text-sm">
                Import Save
              </button>
            </div>
          </div>
        ...
        {!savedGame && onImportSave && (
          <div className="w-full max-w-md mx-auto mb-8">
            <button onClick={handleImport} className="w-full py-2.5 border border-parchment-dark/20 text-parchment-dark/60 font-body rounded hover:border-war-gold/40 hover:text-war-gold/80 transition-colors cursor-pointer text-sm">
              Import Save File
            </button>
          </div>
        )}
            ...
            return (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-sm md:text-base text-parchment/70 mb-4 leading-relaxed font-body">
                    {faction.description}
                  </p>
                  <div className="text-sm text-parchment/50 space-y-1 font-body">
                    <p><span className="text-war-gold/80 font-semibold">Leaders:</span> {faction.leaders}</p>
                    <p><span className="text-war-gold/80 font-semibold">Bonus:</span> {faction.bonus}</p>
                  </div>
                </div>
        ...
        <div className="flex flex-col gap-4 items-center animate-slideup" style={{ animationDelay: '0.3s' }}>
          {onStartLearning && (
            <button
              ...
              className="px-8 py-2.5 rounded font-body text-base border border-war-gold/30 text-war-gold/80
        ...
        {/* Footer */}
        <p className="text-center text-sm text-parchment-dark/40 mt-10 max-w-lg mx-auto italic font-body leading-relaxed">
          June 18, 1812 — President Madison signs the declaration of war against Great Britain.
          The young republic faces the world's greatest naval power.
        </p>
        <p className="text-center text-xs text-parchment-dark/25 mt-4 font-body">
          v1.1.0
        </p>
      </div>
    </div>
